<?php
/**
 * good_theme functions and definitions
 *
...
if ( ! function_exists( 'good_theme_setup' ) ) :
	...
	function good_theme_setup() {
		 ...
		 * If you're building a theme based on good_theme, use a find and replace
		 * to change 'good_theme' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'good_theme', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'good_theme' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'good_theme_custom_background_args',
				array(
 ...
 */
function good_theme_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'good_theme_content_width', 640 );
}
add_action( 'after_setup_theme', 'good_theme_content_width', 0 );
...
function good_theme_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'good_theme' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'good_theme' ),
...
add_action( 'wp_enqueue_scripts', 'good_theme_scripts' );
